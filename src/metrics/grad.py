...
@torch.no_grad()
def grad_mean(module: torch.nn.Module) -> Dict[str, Tuple[int, float, float]]:
    def _mean(p):
        return np.prod(p.grad.data.shape), p.grad.data.mean().item()

    mean = {name: _mean(p) for name, p in module.named_parameters() if p.grad is not None}
...
@torch.no_grad()
def grad_minmax(module: torch.nn.Module) -> Dict[str, Tuple[float, float, float]]:
    def _mm(p):
        return p.grad.data.min(), p.grad.data.max()

    mm = {name: _mm(p) for name, p in module.named_parameters() if p.grad is not None}
...
class GradientMean(Metric):
    ...
    def from_config(cls, cfg):
        ...
        key = cfg.get('key', 'GradientMean/')
        parameters = cfg.get('parameters', 'total')

        return cls(key, parameters)

    def __init__(self, key: str = 'GradientMean/', params: Union[str, List[str]] = 'total'):
        super().__init__()
...
class GradientMinMax(Metric):
    ...
    def from_config(cls, cfg):
        ...
        key = cfg.get('key', 'GradientMinMax/')
        parameters = cfg.get('parameters', 'total')

        return cls(key, parameters)

    def __init__(self, key: str = 'GradientMinMax/', params: Union[str, List[str]] = 'total'):
        super().__init__()
