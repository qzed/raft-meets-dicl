...
class FeatureEncoder(nn.Module):
    ...
    def __init__(self, output_dim=32, norm_type='batch', dropout=0.0, relu_inplace=True):
        ...
        self.layer6_center = nn.Sequential(     # (H/32, W/32, 192) -> (H/64, H/64, 224)
            RfpmRfdBlock(192, 224, norm_type, stride=2, relu_inplace=relu_inplace),
            ResidualBlock(224, 224, norm_type, stride=1, relu_inplace=relu_inplace),
        )
        ...
        self.out6 = RfpmOutputNet(3*224, output_dim, 3*256, norm_type=norm_type, dropout=dropout, relu_inplace=relu_inplace)
