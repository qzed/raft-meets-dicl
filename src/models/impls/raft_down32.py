# Downsized RAFT for baseline comparisons. Essentially RAFT without upsampling
# network and internal dimensions H/32, W/32 instead of H/8, W/8.
...
class CorrBlock:
    ...
    def __call__(self, coords):
        ...
        delta = torch.stack(torch.meshgrid(dx, dy, indexing='ij'), axis=-1)  # to (2r+1, 2r+1, 2)
...
class RaftModule(nn.Module):
    ...
    def initialize_flow(self, img):
        ...
        cy = torch.arange(h // 32, device=img.device)
        cx = torch.arange(w // 32, device=img.device)
    ...
    def forward(self, img1, img2, iterations=12, flow_init=None):
        ...
        for _ in range(iterations):
            ...
            flow_up = self.upsample(coords1 - coords0, shape=img1.shape)
